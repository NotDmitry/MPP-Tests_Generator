...
{
    public async Task Generate(string sourcePath, int maxReadCount, int maxProcessCount, int maxWriteCount, string destinationPath)
    {
        ...
        var writeRoutine = new ActionBlock<List<TargetContainer>>(code => WriteTestsToFile(Path.Combine(destinationPath, "AutogeneratedMSTests"), code),
